...
def get_conditions(filters):
    ...
    if filters.get("warehouse"):
        conditions += " and sle.warehouse = '%s'" % filters.get("warehouse")
    elif filters.get("warehouse_group"):
        conditions += (
            " and sle.warehouse in (select name from `tabWarehouse` wh where wh.parent_warehouse = '%s')"
            % filters.get("warehouse_group")
        )
...
def get_item_warehouse_map(filters, sle):
    ...
    for d in sle:
        ...
        if d.posting_date < from_date or (
            ...
            and frappe.db.get_value("Stock Reconciliation", d.voucher_no, "purpose")
            == "Opening Stock"
        ):
...
def get_items(filters):
    ...
    if filters.get("item_code"):
        ...
        if filters.get("item_category_2"):
            conditions.append("item.item_category_2=%(item_category_2)s")
...
def get_item_details(items, sle, filters):
    ...
    res = frappe.db.sql(
        """
        select
            item.name, item.item_name, item.description, item.item_group, item.item_category, item.item_category_2, item.brand, item.stock_uom %s
        from
            `tabItem` item
            %s
        where
