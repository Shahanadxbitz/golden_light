...
def get_items(filters):
    ...
    if filters.get("item_code"):
        ...
        if filters.get("item_category"):
            conditions.append("item_category=%(item_category)s")
        if filters.get("item_category_2"):
            conditions.append("item_category_2=%(item_category_2)s")
...
def get_item_details(items, sl_entries, include_uom):
    ...
    res = frappe.db.sql(
        """
        select
            item.name, item.item_name, item.description, item.item_group, item.item_category, item.item_category_2, item.brand, item.stock_uom {cf_field}
        from
            `tabItem` item
            {cf_join}
        where
...
def get_sle_conditions(filters):
    conditions = []
    if filters.get("warehouse") or filters.get("warehouse_group"):
        warehouse_condition = get_warehouse_condition(
            filters.get("warehouse"), filters.get("warehouse_group")
        )
...
def get_warehouse_condition(warehouse, warehouse_group):

    conditions = ""

    if warehouse:
        conditions += "warehouse = '%s'" % warehouse
    elif warehouse_group:
        conditions += (
            "warehouse in (select name from `tabWarehouse` wh where wh.parent_warehouse = '%s')"
            % warehouse_group
        )
